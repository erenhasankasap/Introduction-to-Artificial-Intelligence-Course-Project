			...
			if(child!=NULL){
				...
				switch(method){
					...
					case GeneralizedAStarSearch:
    						child->state.h_n = Compute_Heuristic_Function(
                           							&(child->state), goal_state);
    						if(temp_node != NULL){
       							float f_child = (1-alpha)*child->path_cost
                     			 			  + alpha*child->state.h_n;
       							float f_temp  = (1-alpha)*temp_node->path_cost
                      						  + alpha*temp_node->state.h_n;
        						if(f_child < f_temp)
       				    			Remove_Node_From_Frontier(temp_node, &frontier);
      							else
       				     			continue;
                                               ...
                                               float alpha)
{
    ...
    Queue *temp_queue;
    Queue *new_queue = (Queue*)malloc(sizeof(Queue));
    if(new_queue==NULL) Warning_Memory_Allocation();

    new_queue->node = child;
    float f_child = (1-alpha)*child->path_cost + alpha*child->state.h_n;
    ...
    if(Empty(*frontier)){
        ...
        *frontier = new_queue;
        ...
        float f_front = (1-alpha)*(*frontier)->node->path_cost 
        ...
        if(f_child < f_front){
            ...
            *frontier = new_queue;
            ...
            for(temp_queue = *frontier; temp_queue->next != NULL; 
                temp_queue = temp_queue->next)
            {
                float f_next = (1-alpha)*temp_queue->next->node->path_cost
